"""
Validação rápida: Principais fornecedores
"""
import sys
import os
...
def test_validacao():
    """Valida apenas janeiro 2026"""
    print("=" * 80)
    print("VALIDACAO RAPIDA - Principais fornecedores")
    print("=" * 80)
            ...
            for i, (fornecedor, dados) in enumerate(fornecedores_ordenados[:5], 1):
                ...
                for nome_ia, valor_ia in fornecedores_ia.items():
                    if nome_ia.upper() in fornecedor.upper():
                        ...
                        if diferenca < 1:
                            print(f"   [OK] Match com IA!")
                            matches += 1
                        else:
                            print(f"   [X] IA: R$ {valor_ia:,.2f}, Dif: R$ {diferenca:,.2f}")
                        break

            print(f"\n{'=' * 80}")
            if matches == 5:
                print("[OK] TODOS OS 5 FORNECEDORES ESTAO CORRETOS!")
            else:
