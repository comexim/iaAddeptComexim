"""
Valida a NOVA resposta da IA sobre contratos baixados
"""
import sys
import os
...
def test_nova_resposta_baixados():
    """Compara nova resposta da IA com dados reais"""
    print("=" * 80)
    print("VALIDACAO - NOVA RESPOSTA DA IA SOBRE CONTRATOS BAIXADOS")
    print("=" * 80)
    ...
    try:
        ...
        # Consulta vendas de janeiro 2026 (embarque)
        print("2. Consultando vendas com embarque em janeiro 2026...")
        ...
        print(f"Total de contratos no banco: {len(baixados_banco)}")
        ...
        for cliente_ia, contratos_ia in ia_respondeu.items():
            ...
            if cliente_encontrado:
                ...
                print(f"\n{cliente_ia_norm}: [ERRO] Cliente nao encontrado!")
        ...
        taxa_acerto_contratos = (contratos_corretos / len(baixados_banco) * 100) if baixados_banco else 0

        print(f"Clientes: {clientes_corretos}/{len(por_cliente_banco)} corretos ({taxa_acerto_clientes:.1f}%)")
        print(f"Contratos: {contratos_corretos}/{len(baixados_banco)} corretos ({taxa_acerto_contratos:.1f}%)")

        if clientes_faltando:
            print(f"\nClientes nao mencionados: {len(clientes_faltando)}")
